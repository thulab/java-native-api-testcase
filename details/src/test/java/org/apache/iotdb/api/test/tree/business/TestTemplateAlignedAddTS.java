...
public class TestTemplateAlignedAddTS extends BaseTestSuite_TreeModel {
    ...
    public void afterClass() throws IoTDBConnectionException, StatementExecutionException {
        session.deleteStorageGroup(database);
        if (checkTemplateExists(templateNames[0])) {
            session.dropSchemaTemplate(templateNames[0]);
        }
    }
...
//        }
    ...
    public void testAddTS() throws IoTDBConnectionException, StatementExecutionException {
        ...
        Assert.assertThrows(StatementExecutionException.class, () -> {
            session.createAlignedTimeseries(devices[0] + ".child", measurement, dataType, encoding, compressor, alias);
        });
        assert 6 == getTimeSeriesCount(devices[0] + ".**", verbose) : "追加一个普通的TS到模版节点失败";
        session.createAlignedTimeseries(database + ".d_normal", measurement, dataType, encoding, compressor, alias);
...
//        schemaList.add(new MeasurementSchema("appendFloat", TSDataType.FLOAT, TSEncoding.GORILLA, CompressionType.SNAPPY));
        doUpdateAfterAddTS_origin(0, true);

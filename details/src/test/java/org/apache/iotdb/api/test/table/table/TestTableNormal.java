package org.apache.iotdb.api.test.table.table;
...
/**
 * Title：测试表操作—正常情况
 * Describe：测试表所有的操作，包括创建表、查看表、查看表结构
 * Author：肖林捷
 * Date：2024/12/29
 */
public class TestTableNormal extends BaseTestSuiteTableModel {
    ...
    public Iterator<Object[]> getData() throws IOException {
        return new CustomDataProvider().load_table("data/table/table_normal.csv", true).getData();
    }
    ...
    public void createTable() throws IoTDBConnectionException, StatementExecutionException, IOException {
        ...
        // 判断是否符合预期
        assert expect == actual : "TestTable_Normal类的createTable方法实际不一致期待：" + expect + "，实际：" + actual;
    }
    ...
    public void showTables() throws IoTDBConnectionException, StatementExecutionException, IOException {
        ...
        while (true) {
            ...
            assert i != showTableNames.size() : " TestTable_Normal类的showTables方法实际表名与期望的不一致，期望的" + createTableNames.get(j) + "不存在";
        }
    }

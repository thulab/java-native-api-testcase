...
/**
 * <p>Title：SessionBuilder接口正常情况的功能测试<p/>
...
public class TestSessionBuilderNormal {
     ...
     */
    @Test(priority = 100)
    public void testSessionPartParam() throws IoTDBConnectionException, StatementExecutionException {
    ...
    public void testSessionOtherMethod() throws IoTDBConnectionException, StatementExecutionException, TException {
        ...
        try (Session session = new Session.Builder()
            ...
            assert session.getQueryTimeout() == 60000 : "期待值和实际不一致，期待：60000，实际：" + session.getQueryTimeout();
    ...
    /**
     * 测试移除损坏的会话连接（只有在启用重定向功能（enableRedirection 为 true）时才执行清理操）
     */
    @Test(priority = 210)
    public void testSessionRemove() throws IoTDBConnectionException, StatementExecutionException {
        try (Session session = new Session.Builder()
                .nodeUrls(Collections.singletonList(config.getValue("host") + ":" + config.getValue("port")))
                .username(config.getValue("user"))
                .password(config.getValue("password"))

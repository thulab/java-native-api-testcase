...
/**
 * <p>Title：测试 InsertAlignedRecord 接口插入数据—正常情况<p/>
 * <p>Describe：测试使用 InsertAlignedRecord 接口插入各种数据类型的数据，包括BOOLEAN、INT32、INT64、FLOAT、DOUBLE、TEXT、STRING、BLOB、TIMESTAMP和DATE<p/>
 * <p>Author：肖林捷<p/>
 * <p>Date：2024/12/29<p/>
 */
public class TestInsertAlignedRecordNormal extends BaseTestSuite_TreeModel {
    // 数据库名称
    ...
    /**
     * 测试 insertAlignedRecord 方法接口（构造方法1）
     */
    @Test(priority = 10)
    ...
    /**
     * 测试 insertAlignedRecord 方法接口（构造方法2）
     */
    @Test(priority = 20)
    public void insertAlignedRecord2() throws IOException, IoTDBConnectionException, StatementExecutionException {
        // insertAlignedRecord(
    ...
    public void compare(int expectAligned) throws IoTDBConnectionException, StatementExecutionException {
        // 获取对齐时间序列的实际记录条数
        int actualAligned = countLines("select * from " + alignedDeviceId, verbose);
        // 断言实际记录条数与预期相等

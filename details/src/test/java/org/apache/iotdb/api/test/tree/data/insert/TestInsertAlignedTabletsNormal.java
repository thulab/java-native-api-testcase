...
/**
 * <p>Title：测试 InsertAlignedTablets 接口插入数据—正常情况<p/>
 * <p>Describe：测试使用 InsertAlignedTablets 接口插入各种数据类型的数据，包括BOOLEAN、INT32、INT64、FLOAT、DOUBLE、TEXT、STRING、BLOB、TIMESTAMP和DATE，含空值<p/>
 * <p>Author：肖林捷<p/>
 * <p>Date：2024/12/29<p/>
 */
public class TestInsertAlignedTabletsNormal extends BaseTestSuite_TreeModel {
    // 数据库名称
    ...
    /**
     * 测试 insertAlignedTables 方法接口（构造方法1）
     */
    ...
    /**
     * 测试 insertAlignedTables 方法接口（构造方法2）
     */
    @Test(priority = 20)
    public void insertAlignedTables2() throws IOException, IoTDBConnectionException, StatementExecutionException {
        // insertAlignedTablets(Map<String, Tablet> tablets)
    }

    /**
     * 测试 insertAlignedTablets 方法接口（构造方法3）
     */
    @Test(priority = 30)
    public void insertAlignedTablets3() throws IOException, IoTDBConnectionException, StatementExecutionException {
        // insertAlignedTablets(Map<String, Tablet> tablets, boolean sorted)
    }
    /**
     * 用于查询比较插入条数看是否和预期相同
     */
    public void compare(int expectAligned) throws IoTDBConnectionException, StatementExecutionException {
        // 获取非对齐时间序列的实际记录条数
        int actualAligned = countLines("select * from " + deviceId, verbose);
        expectAligned = expectAligned - 3;
        // 断言实际记录条数与预期相等
        assert actualAligned == expectAligned : "非对齐：实际记录条数与预期不一致，actual=" + actualAligned + " expect=" + expectAligned;
        // 清理非对齐时间序列的数据

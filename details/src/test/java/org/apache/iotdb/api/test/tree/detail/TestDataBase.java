package org.apache.iotdb.api.test.tree.detail;
...
public class TestDataBase extends BaseTestSuite_TreeModel {
    ...
    public void beforeClass() throws IoTDBConnectionException, StatementExecutionException, IOException {
        normalSGs = new CustomDataProvider().getFirstColumns("data/tree/storage-group.csv");
        deleteSGs = new CustomDataProvider().getFirstColumns("data/tree/storage-group-data-for-delete.csv");
    }
    ...
    private Iterator<Object[]> getDeleteStorageGroupNormalMatch() throws IOException {
        return new CustomDataProvider().load("data/tree/storage-group-delete.csv").getData();
    }
    ...
    private Iterator<Object[]> getDeleteStorageGroupError() throws IOException {
        return new CustomDataProvider().load("data/tree/storage-group-error.csv").load("data/tree/storage-group-delete-error.csv").getData();
    }

    @DataProvider(name = "deleteStorageGroupsError")
    private Iterator<Object[]> getDeleteStorageGroupsError() throws IOException {
        return new CustomDataProvider().load("data/tree/storage-group-deleteG-error.csv").getData();
    }
    ...
    private Iterator<Object[]> getStorageGroupError() throws IOException {
        return new CustomDataProvider().load("data/tree/storage-group-error.csv").getData();
    }

    @DataProvider(name = "normalNames", parallel = true)
    private Iterator<Object[]> getNormalNames() throws IOException {
        return new CustomDataProvider().load("data/tree/keyword-normal.csv").getData();
    }
